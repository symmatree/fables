...
- **RTCM must flow continuously** even under canopy (via ELRS). If the F9P loses RTCM input, RTK convergence after breakout takes much longer because the receiver needs to rebuild the differential corrections from scratch. Keep the ELRS link active and RTCM injecting at all times. How corrections reach the FC, and the leading theory for what silently stops them: coordinator [`docs/rtk-corrections-path.md`](https://github.com/symmatree/coordinator/blob/main/docs/rtk-corrections-path.md).
