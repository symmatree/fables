...
- **RTCM must flow continuously** even under canopy (via ELRS). If the F9P loses RTCM input, RTK convergence after breakout takes much longer because the receiver needs to rebuild the differential corrections from scratch. Keep the ELRS link active and RTCM injecting at all times. How the corrections actually reach the FC, and the leading (unconfirmed) theory for the failure that silently stops them -- the ELRS backpack anchoring to the wrong ground station -- are in coordinator [`docs/rtk-corrections-path.md`](https://github.com/symmatree/coordinator/blob/main/docs/rtk-corrections-path.md).
