...
| ArduPilot configuration (params, serial, RC, tools) | [ardupilot.md](ardupilot.md) ; [`config/rekon10-methodi.param`](config/rekon10-methodi.param) |
...
**`config/`:** EdgeTX and ArduPilot exports live under [`config/`](config/) -- [`MODELS/model01.yml`](config/MODELS/model01.yml), [`MODELS/model02.yml`](config/MODELS/model02.yml), [`RADIO/radio.yml`](config/RADIO/radio.yml), [`rekon10-methodi.param`](config/rekon10-methodi.param). If IDE-wide search returns nothing for those paths, open them by path: some multi-root setups only index a single workspace root, so glob-from-default-root can miss `facts/` even though the files are in git here.
