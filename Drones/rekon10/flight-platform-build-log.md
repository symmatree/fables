...
> **Update (2026-07-30, leading theory -- not yet confirmed).** A read of the ELRS backpack firmware source (master branch, not verified against our Rev 1.5.5) supports the Mission-Planner-contention side and suggests the two hypotheses are the *same* mechanism: the backpack broadcasts for discovery, latches to the **first** GCS that answers, unicasts only there, and never releases the lock -- so a stray Mission Planner/QGC could steal it and a *radio* power-cycle would recover it (clearing the lock, re-running the race). Consistent with both cases, but still no GCS caught red-handed and the backpack's `GET /mavlink` status endpoint (which would name the offender in `ip.gcs`) hasn't been hit on our unit. Current theory, the diagnostic to run next time, and the open items: coordinator [`docs/rtk-corrections-path.md`](https://github.com/symmatree/coordinator/blob/main/docs/rtk-corrections-path.md) and [coordinator#99](https://github.com/symmatree/coordinator/issues/99).
