...
| CH9 | **`RC9_OPTION = 36`** (Relay4). **`RELAY4_PIN = 83`**, **`RELAY4_FUNCTION = 1`** (Lucid HD VTX BEC). **`RELAY4_DEFAULT = 0`** so the VTX BEC is **off at power-up** (bench work with the radio off leaves the VTX cold); CH9 / **SA** controls it when disarmed, and the Boxer **arm gate forces CH9 high (VTX on) while armed** ([ground-station.md](ground-station.md)) -- arming powers the VTX, no manual step. |

**VTX default-off at boot:** **`RELAY4_DEFAULT`** is the relay's startup state (**0 = Off, 1 = On, 2 = No change**). Set to **0** so a bench power-up with **no radio** leaves the VTX unpowered instead of energizing it (was **1**). With the radio on, CH9 controls Relay4 when disarmed; the Boxer **arm-gate mix in [`config/MODELS/model01.yml`](config/MODELS/model01.yml) forces CH9 high (VTX on) whenever armed** (via the L1-L3 latch; [ground-station.md](ground-station.md)), so **arming powers the VTX automatically -- no need to remember to switch it on**, and it can't drop mid-flight. `RELAY4_DEFAULT = 0` therefore only changes the **disarmed / radio-off** state, which is exactly the bench case. **Polarity not yet bench-verified this session:** confirm `RELAY4_DEFAULT = 0` with `RELAY4_INVERTED = 0` actually leaves the BEC de-energized (power up radio-off, check the goggles see no VTX). **`RELAY2_DEFAULT` / `RELAY3_DEFAULT`** (pins 81/82) are still **1** and undocumented here -- left alone, but they also come up energized at boot.
