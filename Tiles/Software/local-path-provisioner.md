...
- **Short summary**: Local Path Provisioner for dynamic local-node storage; provides StorageClass `local-path` as default. Deployed via Helm from containeroo.ch.
- **application.yaml type**: helm
- **Where loaded from**: Helm chart `local-path-provisioner` from https://charts.containeroo.ch
- **Version deployed**: 0.0.32
- **Links**:
  - Application: [local-path-provisioner-application.yaml](https://github.com/symmatree/tiles/blob/main/charts/argocd-applications/templates/local-path-provisioner-application.yaml)
...
No Terraform-managed resources (see **Configuration** above).
