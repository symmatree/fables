...
- **The DJI's -70 degree forward-facing gimbal angle reduces RS relative to nadir.** Forward velocity partially along the camera's line of sight gives ~12% less RS displacement than a pure nadir camera at the same speed (cos^2 geometry). This is relevant as a baseline comparison for the Rekon multi-camera array, where cross-track cameras see slightly more RS (see [arm-pods.md](../Drones/rekon10/arm-pods.md) rolling shutter section).
