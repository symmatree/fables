...
- **Multi-camera synchronized capture creates a local exception to the nearfield parallax problem within each burst (cross-ref Rekon array).** All of the nearfield parallax analysis above assumes single-camera sequential capture where images are taken seconds apart and meters apart in space. The Rekon 10 multi-camera array ([arm-pods.md](https://github.com/symmatree/coordinator/blob/main/docs/rekon10/arm-pods.md)) captures 8 synchronized images within microseconds via PPS, with intra-pod baselines of ~4-5 inches. Within each burst: (a) twig features are frozen at one physical position across all 8 images (no wind movement between frames); (b) nearfield parallax from twigs is proportional to the ~4-5 inch camera baseline rather than the meters of displacement between sequential single-camera captures; (c) forward-and-backward cameras capture opposite sides of trees within seconds of passing overhead rather than from separate passes. However, **between successive bursts** (seconds apart, meters of drone displacement), the full nearfield parallax problem returns -- wind moves twigs, the scene changes, and cross-burst feature tracks through nearfield objects will be unreliable. The synchronized array carves out robust local stitching webs within each capture event, but the connections between these webs still depend on stable features (ground, trunks, structures). The track-length filtering and feature-height strategies developed here remain relevant for cross-burst matching. The Rekon house-orbit experiment (Planned next steps) should test how well the intra-burst webs connect to each other and whether the DJI-era filtering strategies are still needed.
...
### Rolling shutter artifact detection in existing DJI imagery

**Confirmed:**

(none yet -- no RS-specific analysis run)

**Current theories:**

- **Rolling shutter distortion may be present but sub-pixel at this GSD.** The DJI Mini 3 Pro has a rolling shutter sensor. At 5.6 m/s with an estimated ~26-30 ms readout, forward-flight displacement is ~15-17 cm (15-17 pixels at 1 cm GSD). This would produce systematic affine shear (parallelogram distortion) in every image. ODM was never run with `--rolling-shutter` enabled, and no RS artifacts were specifically identified in the diagnostic work so far. The problems documented (autofocus, nearfield parallax, turnaround instability) are not RS artifacts. RS distortion may exist but be absorbed by the bundle adjustment without explicit correction.
- **The DJI's -70 degree forward-facing gimbal angle reduces RS relative to nadir.** Forward velocity partially along the camera's line of sight gives ~12% less RS displacement than a pure nadir camera at the same speed (cos^2 geometry). This is relevant as a baseline comparison for the Rekon multi-camera array, where cross-track cameras see slightly more RS (see [arm-pods.md](https://github.com/symmatree/coordinator/blob/main/docs/rekon10/arm-pods.md) rolling shutter section).
...
- [ ] **Line straightness analysis on close-orbit house images.** Select close-orbit images from 02-21 with prominent architectural edges (house walls, window frames, gutters). Rectify for lens distortion (DJI EXIF provides camera model). Run a line segment detector (LSD) on rectified images. Measure deviation of detected segments from best-fit straight lines. Check for:
  - **Systematic shear** (forward-flight RS): vertical lines lean consistently, and the lean direction rotates with flight heading across images.
  - **Periodic waviness** (vibration jello): sinusoidal deviation from straight, periodic at a frequency related to prop RPM / readout time.
  - Quantify residual magnitudes: if sub-pixel, RS distortion doesn't matter at this GSD.
- [ ] **Reprocess house-2 with `--rolling-shutter` enabled.** Compare reconstruction metrics (reprojection error, GPS ce90, track length, displacement stats) to the house-2 baseline. If RS correction improves metrics, that's evidence RS was present and correctable. If no improvement, RS is not a limiting factor for DJI imagery at these speeds.
- [ ] **Compare images with different flight headings.** If detected line "lean" rotates with heading, that confirms forward-flight RS shear (vs a fixed optical aberration). Grid images from N-S and E-W passes are ideal for this.
- [ ] **Establish quantitative baseline** for "how much RS distortion exists in imagery that already produced usable photogrammetry." This baseline informs expectations for the Rekon array at 3-5 m/s with similar sensors.
...
- [ ] **Rolling shutter line-straightness analysis** on existing DJI close-orbit imagery -- no flight needed (Diagnostic, see RS open issue above)
- [ ] **Reprocess house-2 with `--rolling-shutter`** enabled and compare metrics (Diagnostic / Processing)
- [ ] **Rekon 10 two-pod house orbit** -- first multi-camera capture test using 2 equipped arms (4 cameras). Fly the house close-orbit route adapted for ArduPilot. Assess: image quality, vibration artifacts (line straightness analysis), RS distortion, multi-camera overlap utility. Compare directly to DJI close-orbit results. Also serves as first integration test for PPS timing chain, chrony sync, and libcamera capture pipeline. (Collection / Integration test)
- [ ] **Assess nearfield parallax with synchronized multi-camera capture** -- the nearfield parallax theory, track-length filtering, and feature-contrast hypotheses (see Reconstruction quality open issue) were all developed for single-camera sequential imagery. The Rekon's synchronized 8-camera array with ~4-5 inch intra-pod baselines and microsecond temporal alignment creates a qualitatively different matching environment. The Rekon house-orbit experiment should specifically assess whether nearfield features that are problematic in DJI sequential captures become reliable matches in synchronized multi-camera captures. (Diagnostic, cross-ref Reconstruction quality theories and [arm-pods.md](https://github.com/symmatree/coordinator/blob/main/docs/rekon10/arm-pods.md) temporal advantage section)
